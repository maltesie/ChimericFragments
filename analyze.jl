...
fastqs = is_paired_end ? PairedSingleTypeFiles([(sname*suffix_read1*"fastq.gz", sname*suffix_read1*"fastq.gz") for (sname, _) in samplename_condition]) :
                            SingleTypeFiles([(sname*"fastq.gz", sname*"fastq.gz") for (sname, _) in samplename_condition])
...
types = hcat([srna_type, cds_type, rrna_type, trna_type], additional_types)
...
conditions = Dict(c => [i for (i, info) in enumerate(samplename_condition) if info[2]===c] for c in unique([t[2] for t in samplename_condition_antibarcode]))
